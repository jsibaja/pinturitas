...
insert into public.palette_colors (hex, name_es, name_en, phonetic_es, sort_order) values
  ...
  ('#F97316','naranja','Orange','O-ranch',2),
  ('#FCD34D','amarillo','Yellow','YE-lou',3),
  ('#EAB308','dorado','Gold','gould',4),
  ('#22C55E','verde','Green','griin',5),
  ('#166534','verde oscuro','Dark Green','dark griin',6),
  ('#A3E635','verde limon','Lime','laim',7),
  ('#14B8A6','turquesa','Turquoise','TER-koiz',8),
  ('#38BDF8','celeste','Light Blue','lait blu',9),
  ('#2563EB','azul','Blue','blu',10),
  ('#1E3A8A','azul marino','Navy','NEI-vi',11),
  ('#7C3AED','morado','Purple','PER-pol',12),
  ('#C084FC','violeta','Violet','VAI-o-let',13),
  ('#F472B6','rosado','Pink','pink',14),
  ('#DB2777','fucsia','Magenta','ma-LLEN-ta',15),
  ('#92531F','cafe','Brown','braun',16),
  ('#FFD9B3','piel clara','Peach','piich',17),
  ('#E0A16E','piel','Tan','tan',18),
  ('#8D5524','piel morena','Sienna','si-EN-na',19),
  ('#EAD9B0','beige','Beige','beish',20),
  ('#111111','negro','Black','blak',21),
  ('#6B7280','gris oscuro','Dark Grey','dark grei',22),
  ('#CBD5E1','gris','Grey','grei',23),
  ('#FFFFFF','blanco','White','wait',24);
...
insert into public.worlds (slug, name_es, name_en, sort_order, cover_image_path, color_hex, is_free) values
  ...
  ('ocean','El Oceano','Ocean Animals',5,'covers/ocean.png','#1D4ED8',false),
  ...
  ('vehicles','Vehiculos','Vehicles',9,'covers/vehicles.png','#3B82F6',false),
  ...
  ('numbers','Numeros','Numbers',25,'covers/numbers.png','#7C3AED',false),
...
select 'plans' t, count(*) from public.plans union all select 'colors', count(*) from public.palette_colors union all select 'worlds', count(*) from public.worlds;